import { listAuthorizedSubscribers } from './security/staff-access';
﻿import {
  activityLog,
  analyticsSnapshot,
...
import { buildStaffProjectPatch, MAX_PROJECT_FILE_BYTES } from "./project-workflow";
import { listProjectFiles, listProjectFilesWithLegacy, overlayProjectFiles, saveProjectFile } from "./project-files";
import {
  checkDatabaseHealth,
...
import {
  ...
  getScenarioDraftByIdFromDb,
  listBusinessEventsFromDb,
  listCoreClientsFromDb,
  listCoreDealsFromDb,
  listScenarioDraftsFromDb,
...
async function getInternalProductSubscribers() {
  return (await listAuthorizedSubscribers()).filter(row => ['director','manager'].includes(row.role));
}
  ...
  const buildFile = (slot) => {
    ...
    if (stored?.pathname) {
      return {
        ...
        url: stored.pathname.startsWith("/") ? stored.pathname : buildPrivateBlobProxyUrl(stored.pathname),
...
export async function uploadLeadProjectAsset(slug, formData) {
  try {
    return await uploadLeadProjectAssetInternal(slug, formData);
  } catch (error) {
    console.warn("Project upload failed:", error.code || "project_storage_error");
    return { ok: false, status: 503, message: "Не удалось сохранить файл. Проверьте хранилище версий и повторите загрузку." };
  }
}

async function uploadLeadProjectAssetInternal(slug, formData) {
  ...
  if (uploadedFile.size > MAX_PROJECT_FILE_BYTES) {
  ...
  const storedFileName = `v-${randomUUID()}${getProjectUploadExtension(fileName, slot)}`;
  const storedAt = new Date().toLocaleString("ru-RU");
  const blobPathname = buildProjectFilePathname(safeSlug, storedFileName);
  const prior = await listProjectFiles(slug);
  let expectedCurrentId = formData.has("expectedCurrentId")
    ? String(formData.get("expectedCurrentId") || "") || null
    : prior.find((file) => file.groupKey === slotId && file.isCurrent)?.id || null;
  if (!prior.some((file) => file.groupKey === slotId)) {
    const imported = await listProjectFilesWithLegacy(lead, slotId);
    const archived = imported.find((file) => file.groupKey === slotId && file.sourceRef === `legacy:${slotId}`);
    if (archived && expectedCurrentId === null) expectedCurrentId = archived.id;
  }

  if (process.env.VERCEL && !isBlobStoreEnabled()) {
    return { ok: false, message: "Хранилище файлов не настроено. Загрузите файл через Telegram." };
  }
  ...
  try {
    await saveProjectFile({
      leadId: slug, groupKey: slotId, expectedCurrentId,
      sourceRef: String(formData.get("sourceRef") || storedFileName),
      fileName, size: buffer.length, contentType: uploadedFile.type || "application/octet-stream",
      kind: slot.preview ? "photo" : "document", pathname: isBlobStoreEnabled() ? blobPathname : `/${blobPathname}`,
      uploadedBy: String(formData.get("uploadedBy") || lead.manager || "Не указан")
    });
  } catch (error) {
    return { ok: false, status: error.code === "PROJECT_VERSION_CONFLICT" ? 409 : 503, message: error.message };
  }
  ...
  const projectAssetsPatch = buildProjectAssetUploadPatch(slot, metadata, {
    pathname: isBlobStoreEnabled() ? blobPathname : `/${blobPathname}`,
...
export async function deleteLeadProjectAsset(slug, payload = {}) {
  ...
  if ((await listProjectFiles(slug, { optional: true })).some((file) => file.groupKey === slotId)) {
    return { ok: false, message: "Для этого файла хранится история. Загрузите новую версию вместо удаления." };
  }
  const slot = getProjectFileSlot(slotId);
  ...
  if (isLiveDatabaseEnabled() && getCompanyId()) {
    try {
        ...
        leadIds.length
          ? query(
              `
                select
                  e.lead_id,
                  jsonb_agg(e.payload order by e.created_at, e.id) as payloads
                from lead_events e
      ...
      const contextPatchMap = new Map(
        contextResult.rows.map((row) => [row.lead_id,
          row.payloads.reduce((patch, payload) => mergeRecord(patch, payload.context_patch || {}), {})])
      );
...
export async function getScenarioDraftsData(options = {}) {
  const companyId = getCompanyId();
  const cacheKey = `scenario-drafts:${companyId || "mock"}:${String(options.status || "all")}:${Number(options.limit || 100)}`;
  ...
  if (isLiveDatabaseEnabled() && companyId) {
    try {
      const drafts = await listScenarioDraftsFromDb(companyId, options);
      writeRuntimeCache(cacheKey, drafts);
      return drafts;
    } catch (error) {
      console.warn("Scenario drafts live data fallback:", error.message);
    }
  }
...
export async function getScenarioDraftByIdData(draftId) {
  const companyId = getCompanyId();

  if (isLiveDatabaseEnabled() && companyId && draftId) {
    try {
      return await getScenarioDraftByIdFromDb(companyId, draftId);
    } catch (error) {
      console.warn("Scenario draft detail live data fallback:", error.message);
    }
  }
...
export async function getLeadBySlug(slug) {
  ...
  if (cached) {
    return overlayProjectFiles(cached);
  }
  ...
  if (isLiveDatabaseEnabled() && getCompanyId()) {
    try {
      ...
      writeRuntimeCache(cacheKey, result);
      return overlayProjectFiles(result);
    } catch (error) {
  ...
  return overlayProjectFiles(fallbackResult);
}
...
export async function createTelegramClientLead(payload = {}) {
  ...
  if (!name || (!phone && !staffRequest) || !product) {
    return {
      ok: false,
  ...
  if (isRuntimeStatePostgresEnabled(getCompanyId())) {
    try {
      const existingReference = staffRequest ? null : await findTelegramLeadReferenceInDb(getCompanyId(), {
      ...
      if (createdLead.created) {
        await recordLeadLifecycleAnalytics({
          ...
          firstName: staffRequest ? payload.authorName || null : payload.name || null,
  ...
  const existingLead = staffRequest
    ? getMockExtraLeads().find((lead) => lead.sourceRef === payload.sourceRef)
    : findMockLeadByTelegramClientReference({
    chatId: payload.chatId,
    telegramUserId: payload.telegramUserId,
    username: payload.username,
    reference: phone
  ...
  const nextLead = buildTelegramClientLeadRecord(staffRequest ? { ...payload, chatId: null, telegramUserId: null, username: null } : payload);
  if (staffRequest) {
    Object.assign(nextLead, buildStaffProjectPatch(payload), { sourceRef: payload.sourceRef });
    nextLead.tasks = nextLead.tasks.map((task) => ({ ...task, owner: payload.manager }));
  }
