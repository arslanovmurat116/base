...
function getMockLeadPatches() {
  if (!globalForDisetCache.__disetMockLeadPatches) {
    ...
    try {
      if (fs.existsSync(MOCK_LEAD_PATCHES_FILE)) {
        persisted = JSON.parse(fs.readFileSync(MOCK_LEAD_PATCHES_FILE, "utf-8"));
      }
    } catch (error) {
      console.warn("Mock lead patch store read failed:", error.message);
    }

    globalForDisetCache.__disetMockLeadPatches = new Map(Object.entries(persisted));
  }

  return globalForDisetCache.__disetMockLeadPatches;
}

function persistMockLeadPatches() {
  try {
    fs.mkdirSync(MOCK_STATE_DIR, { recursive: true });
    const payload = Object.fromEntries(getMockLeadPatches().entries());
    fs.writeFileSync(MOCK_LEAD_PATCHES_FILE, JSON.stringify(payload, null, 2), "utf-8");
  } catch (error) {
...
function writeMockLeadPatch(slug, patch) {
...
function getMockLeadDataset() {
  return clone(leadsTable).map((lead) => applyMockLeadPatch(lead));
}
...
function updateMockLeadCollection(slug, collectionKey, updater) {
...
function findMockLeadSlug(name) {
  ...
  const exactMatch = leadsTable.find((lead) => {
  ...
  const partialMatch = leadsTable.find((lead) => {
...
function buildProjectAssetUploadPatch(slot, metadata = {}) {
...
export async function uploadLeadProjectAsset(slug, formData) {
  ...
  writeMockLeadPatch(slug, projectAssetsPatch);
  invalidateRuntimeCache();
...
export async function getLeadsData() {
  ...
  const fallback = clone(leadsTable).map((lead) => buildFurnitureLead(applyMockLeadPatch(lead)));
...
export async function updateLeadWorkflow(slug, payload) {
  ...
  writeMockLeadPatch(slug, mockPatch);
  invalidateRuntimeCache();
...
export async function createAppointment(payload) {
  ...
  const nextAppointments = updateMockLeadCollection(mockLead.slug, "appointments", (items) => [
    ...items,
    {
      id: appointmentId,
      lead: mockLead.name,
  ...
  writeMockLeadPatch(mockLead.slug, {
    status: "MEETING",
...
export async function updateAppointmentStatus(payload) {
  ...
  writeMockLeadPatch(mockLead.slug, workflowPatch);
  invalidateRuntimeCache();
...
export async function createFollowup(payload) {
  ...
  const nextFollowups = updateMockLeadCollection(mockLead.slug, "followups", (items) => [
    ...items,
    {
...
export async function getWorkboardData(role = "owner") {
  ...
  const filteredLeads = dashboard.queue.filter((item) => {
    return true;
  });
  ...
  const filteredAlerts = activity.filter((item) => {
    return true;
  });
  ...
  const focus = [
    {
      ...
      value: String(
        filteredLeads.filter((item) => item.status === "NEW").length
      ),
    ...
    },
    {
      ...
      value: String(
        flatTasks.filter((item) => item.lane === "Срочно").length
      ),
  ...
  const mapLeadLink = (item) => ({
    ...
    slug:
      item.slug ||
      findMockLeadSlug(
        item.lead || item.title || item.note || item.action || item.detail
      )
  });
  ...
  if (!filteredLeads.length && !flatTasks.length && !filteredFollowups.length) {
    ...
    return {
      ...fallback,
      urgentLeads: fallback.urgentLeads.map(mapLeadLink),
      taskQueue: fallback.taskQueue.map(mapLeadLink),
      followups: fallback.followups.map(mapLeadLink),
      alerts: fallback.alerts.map(mapLeadLink)
    };
  }
