...
async function getInternalProductSubscribers() {
  const stored = await readPersistentJson(TELEGRAM_SUBSCRIBERS_FILE, []);
...
export async function getPilotRequestsData(limit = null) {
  let freshRequests = await readPersistentJson(
    TELEGRAM_PILOT_REQUESTS_FILE,
    getMockPilotRequests()
  );
...
export async function getProductLaunchesData(limit = null) {
  const freshLaunches = await readPersistentJson(
    PRODUCT_LAUNCHES_FILE,
    getMockProductLaunches()
  );
...
export async function getCustomerSuccessData(limit = null) {
  const freshSuccess = await readPersistentJson(
    CUSTOMER_SUCCESS_FILE,
    getMockCustomerSuccessLoops()
  );
...
async function ensureProductLaunchForPilotRequest(request) {
  const storedLaunches = await readPersistentJson(
    PRODUCT_LAUNCHES_FILE,
    getMockProductLaunches()
  );
  ...
  if (existingLaunchIndex >= 0) {
    ...
    globalForDisetCache.__disetMockProductLaunches = nextLaunches;
    await writePersistentJson(PRODUCT_LAUNCHES_FILE, nextLaunches);
  ...
  return {
    launch,
    created: true,
    notifications: await notifyInternalTeamAboutProductLaunchCreated(launch)
  };
}

async function ensureCustomerSuccessForLaunch(launch) {
  const storedSuccess = await readPersistentJson(
    CUSTOMER_SUCCESS_FILE,
    getMockCustomerSuccessLoops()
  );
  ...
  if (existingIndex >= 0) {
    ...
    globalForDisetCache.__disetMockCustomerSuccess = nextSuccess;
    await writePersistentJson(CUSTOMER_SUCCESS_FILE, nextSuccess);
  ...
  return {
    success,
    created: true,
    notifications: await notifyInternalTeamAboutCustomerSuccessCreated(success)
  };
}
...
export async function updatePilotRequestStatus(payload = {}) {
  ...
  const storedRequests = await readPersistentJson(
    TELEGRAM_PILOT_REQUESTS_FILE,
    getMockPilotRequests()
  );
  ...
  globalForDisetCache.__disetMockPilotRequests = nextRequests;
  await writePersistentJson(TELEGRAM_PILOT_REQUESTS_FILE, nextRequests);
  invalidateRuntimeCache();
  ...
  if (status === "WON") {
    const launchResult = await ensureProductLaunchForPilotRequest(updatedRequest);
  ...
  return {
    ...
    launchCreated,
    launchNotifications,
    message: `Статус ${updatedRequest.requestNumber} обновлён`
  };
}
...
export async function updateProductLaunchStatus(payload = {}) {
  ...
  if (status === "LIVE") {
    const successResult = await ensureCustomerSuccessForLaunch(updatedLaunch);
  ...
  return {
    ...
    message: `Статус ${updatedLaunch.launchNumber} обновлён`
  };
}
...
export async function updateCustomerSuccessStatus(payload = {}) {
  ...
  return {
    ...
    success: updatedSuccess,
    notifications,
    message: `Статус ${updatedSuccess.successNumber} обновлён`
  };
}
