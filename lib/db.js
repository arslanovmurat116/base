...
function hasDatabaseConfig() {
  return getDatabaseConfigState().hasRequiredConfig;
}
...
export function getDatabaseConfigState() {
  const hasHost = hasEnvValue("POSTGRES_HOST");
  const hasPort = hasEnvValue("POSTGRES_PORT");
  const hasDatabase = hasEnvValue("POSTGRES_DATABASE");
  const hasUser = hasEnvValue("POSTGRES_USER");
  const hasPassword = hasEnvValue("POSTGRES_PASSWORD");
  const hasSsl = hasEnvValue("POSTGRES_SSL");
  const requiredMissing = [];

  if (!hasHost) {
    requiredMissing.push("POSTGRES_HOST");
  }

  if (!hasDatabase) {
    requiredMissing.push("POSTGRES_DATABASE");
  }

  if (!hasUser) {
    requiredMissing.push("POSTGRES_USER");
  }

  if (!hasPassword) {
    requiredMissing.push("POSTGRES_PASSWORD");
  }
  ...
  return {
    ...
    hasPassword,
    hasSsl,
    hasAnyConfig: hasHost || hasPort || hasDatabase || hasUser || hasPassword || hasSsl,
    hasRequiredConfig: requiredMissing.length === 0,
    requiredMissing,
    sslEnabled: process.env.POSTGRES_SSL === "true"
  };
}
