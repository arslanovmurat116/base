...
export { STAFF_ROLES, isStaffRole } from './access-policy.js';
...
export async function createBoseSessionToken(claims = {}) {
  ...
  const payload = {
    ...
    telegramUserId: String(claims.telegramUserId || ''),
    exp: Number.isFinite(expiresAt) ? expiresAt : Date.now() + 60 * 60 * 1000
  };

  if (!payload.sid || !/^[1-9]\d{0,19}$/.test(payload.telegramUserId) || payload.exp <= Date.now()) return null;
...
export async function verifyBoseSessionToken(value) {
  ...
  try {
    ...
    if (
      payload?.v !== 2 ||
      !payload?.sid ||
      !/^[1-9]\d{0,19}$/.test(payload?.telegramUserId) ||
