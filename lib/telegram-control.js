...
function buildClientHelpText(lead = null) {
  return [
    "Telegram-бот мебельного цеха",
    "",
    lead?.slug
      ? `Ваш заказ найден: ${lead.name}${lead.orderNumber ? ` (${lead.orderNumber})` : ""}`
      : "Здесь можно быстро оставить заявку или посмотреть текущий статус заказа.",
    "",
...
async function notifyInternalTeamAboutClientLead(lead) {
  ...
  const subscribers = getTelegramSubscribers().filter((item) =>
    ["director", "manager"].includes(item.role)
  );
...
export async function handleTelegramCommand(text, from, chat) {
  ...
  if (
    !currentSubscriber &&
    ["request", "order", "estimate", "deposit", "manager"].includes(command)
  ) {
    const intentMap = {
      request: "request",
      order: "status",
