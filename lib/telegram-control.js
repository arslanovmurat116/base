...
async function persistTelegramPilotRequest(request) {
  const current = getTelegramPilotRequests();
...
function buildClientHelpText(lead = null) {
  return [
    "Furneq — Telegram CRM для мебельного цеха",
    "",
    lead?.slug
...
function buildClientDemoTextV2() {
  return [
    "РљР°Рє РїРѕРєР°Р·С‹РІР°С‚СЊ Furneq РІ РґРµРјРѕ:",
    "",
...
function buildPilotRequestId() {
  return `PILOT-${String(getTelegramPilotRequests().length + 1).padStart(3, "0")}`;
}
...
async function notifyInternalTeamAboutPilotRequest(request) {
  ...
  const text = [
    "Новая заявка на запуск пилота Furneq",
...
async function startPilotRequestFlow(chatId, from) {
  ...
  return deliverTelegramMessage(
    chatId,
    [
      "Запускаем заявку на пилот Furneq.",
      "",
...
async function handlePilotRequestStep(messageText, chat, from, pendingState) {
  ...
  const requestNumber = buildPilotRequestId();
  const request = {
  ...
  await persistTelegramPilotRequest(request);
  await clearPendingRegistration(chat?.id);
  ...
  return {
    ...
    request,
    internalNotifications: await notifyInternalTeamAboutPilotRequest(request),
    reply: await deliverTelegramMessage(
      chat?.id,
      [
