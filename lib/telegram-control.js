...
async function handleClientIntent(intent, chat, from, resolvedLead = null) {
  ...
  if (intent === "demo") {
    return {
      ok: true,
      action: "client_demo",
      reply: await deliverTelegramMessage(chat?.id, buildClientDemoText(), {
        replyMarkup: buildClientDemoReplyMarkup(lead),
        forcePreview: !isTelegramBotConfigured(),
        appButtonLabel: "Открыть Mini App"
      })
    };
...
export async function handleTelegramCommand(text, from, chat) {
    ...
    const reply = currentSubscriber
      ? await deliverTelegramMessage(
          ...
          ].join("\n"),
          {
            replyMarkup: buildInlineKeyboard(
              [
                [buildOpenDealButton("l-202", "summary", "Открыть пример сделки")],
      ...
      : await deliverTelegramMessage(chat?.id, buildClientDemoText(), {
          replyMarkup: buildClientDemoReplyMarkup(currentClientLead),
          forcePreview: !isTelegramBotConfigured(),
          appButtonLabel: "Открыть Mini App"
        });
  ...
  if (
    !currentSubscriber &&
    ["request", "order", "estimate", "deposit", "manager", "demo"].includes(command)
  ) {
    const intentMap = {
      request: "request",
      order: "status",
...
export async function handleTelegramWebhookUpdate(update) {
  ...
  if (messageText && pendingRegistration?.flow?.startsWith("client_")) {
