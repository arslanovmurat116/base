...
const TELEGRAM_SUBSCRIBERS_FILE = path.join(
  MOCK_STATE_DIR,
  "telegram-subscribers.json"
);
const TELEGRAM_DISPATCH_LOG_FILE = path.join(
  MOCK_STATE_DIR,
  "telegram-dispatch-log.json"
);
const TELEGRAM_REGISTRATION_STATE_FILE = path.join(
  MOCK_STATE_DIR,
  "telegram-registration-state.json"
);
...
function readJsonFile(filePath, fallbackValue) {
  try {
    ...
    console.warn(`Telegram control store read failed for ${path.basename(filePath)}:`, error.message);
    return fallbackValue;
  }
}
...
function writeJsonFile(filePath, value) {
  try {
    ...
    console.warn(`Telegram control store write failed for ${path.basename(filePath)}:`, error.message);
  }
}
...
function getStoredTelegramSubscribers() {
  const stored = readJsonFile(TELEGRAM_SUBSCRIBERS_FILE, []);
...
function upsertTelegramSubscriber(subscriber) {
  ...
  writeJsonFile(TELEGRAM_SUBSCRIBERS_FILE, next);
  return subscriber;
}

function getTelegramDispatchLog() {
  const stored = readJsonFile(TELEGRAM_DISPATCH_LOG_FILE, {});
  return stored && typeof stored === "object" ? stored : {};
}

function getTelegramRegistrationState() {
  const stored = readJsonFile(TELEGRAM_REGISTRATION_STATE_FILE, {});
  return stored && typeof stored === "object" ? stored : {};
}

function writeTelegramRegistrationState(nextState) {
  writeJsonFile(TELEGRAM_REGISTRATION_STATE_FILE, nextState);
}
...
function writePendingRegistration(chatId, payload) {
  if (!chatId) {
    return;
  }
  ...
  writeTelegramRegistrationState(nextState);
}

function clearPendingRegistration(chatId) {
  if (!chatId) {
    return;
  }
...
function markDispatchKeySent(key) {
  if (!key) {
    return;
  }
  ...
  writeJsonFile(TELEGRAM_DISPATCH_LOG_FILE, log);
}
...
function buildFollowupActionMessage(item, subscriber, now) {
  ...
  return {
    key: `followup:${item.slug}:${item.typeKey || "custom"}:${subscriber.chatId}:${dateKey}`,
    ttlHours: 4,
    text: [
...
function buildAppointmentActionMessage(item, subscriber, now) {
  ...
  return {
    key: `appointment:${item.id}:${subscriber.chatId}:${now.toISOString().slice(0, 13)}`,
    ttlHours: 4,
    text: [
      "Контроль замера или встречи",
      `${item.lead}`,
...
function buildDispatchMessagesForSubscriber(data, subscriber) {
  const messages = [];
  ...
  if (subscriber.role === "manager") {
    ...
    for (const item of followups) {
      messages.push(buildFollowupActionMessage(item, subscriber, data.now));
    }

    for (const item of appointments) {
      messages.push(buildAppointmentActionMessage(item, subscriber, data.now));
    }
...
export async function runTelegramDispatch(options = {}) {
  ...
  for (const subscriber of subscribers) {
    const messages = buildDispatchMessagesForSubscriber(data, subscriber);

    for (const message of messages) {
      if (!dryRun && !canSendDispatchKey(message.key, message.ttlHours || 4)) {
        deliveries.push({
      ...
      if (!dryRun && delivery.mode === "sent") {
        markDispatchKeySent(message.key);
      }
...
export async function handleTelegramCommand(text, from, chat) {
  ...
  if (command === "register") {
    ...
    if (!role) {
      writePendingRegistration(chat?.id, {
        flow: "register",
    ...
    upsertTelegramSubscriber(subscriber);
    clearPendingRegistration(chat?.id);
    ...
    const reply = await deliverTelegramMessage(
      ...
      [
        ...
        `Имя: ${name}`,
        "",
        "Теперь доступны /today, /alerts, /appointments и кнопки контроля."
      ].join("\n"),
      {
        forcePreview: !isTelegramBotConfigured()
      }
...
export async function handleTelegramCallbackQuery(callbackQuery) {
  ...
  if (kind === "reg" && action === "role") {
    ...
    writePendingRegistration(callbackQuery?.message?.chat?.id, {
      flow: "register",
      role,
...
export async function handleTelegramWebhookUpdate(update) {
  ...
  if (
    ...
    upsertTelegramSubscriber(subscriber);
    clearPendingRegistration(update?.message?.chat?.id);
    ...
    const reply = await deliverTelegramMessage(
      ...
      [
        ...
        `Имя: ${subscriber.name}`,
        "",
        "Теперь доступны /today, /alerts и /appointments."
      ].join("\n"),
      {
        forcePreview: !isTelegramBotConfigured()
      }
