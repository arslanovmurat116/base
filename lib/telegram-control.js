...
function getTelegramRoleLabel(role) {
  return getUnifiedTelegramRoleLabel(role, "en");
...
async function buildCommandMessages(command, subscriber) {
  ...
  switch (command) {
    ...
    case "digest":
      if (!subscriber) {
        return [{ text: "Сначала привяжи чат через /register директор|менеджер|замерщик|дизайнер Имя." }];
      }
...
export async function handleTelegramCommand(text, from, chat) {
  ...
  if (command === "status") {
    ...
    const textReply = currentSubscriber
      ? [
          ...
          .filter(Boolean)
          .join("\n")
      : "Чат ещё не привязан. Используй /register директор|менеджер|замерщик|дизайнер Имя.";
  ...
  if (command === "register") {
    ...
    if (!role) {
      ...
      const reply = await deliverTelegramMessage(
        chat?.id,
        "Выбери роль кнопкой ниже или напиши команду целиком: /register директор Имя, /register менеджер Имя, /register замерщик Имя, /register дизайнер Имя.",
        {
    ...
    const englishRegisterReply = await deliverTelegramMessage(
      chat?.id,
      [
        "Chat linked to role.",
        `Role: ${getTelegramRoleLabel(role)}`,
        `Name: ${name}`,
        "",
        "You can now use /today, /control, /alerts, /appointments, and the BOSE scenario commands."
      ].join("\n"),
      {
        replyMarkup: buildStaffReplyKeyboard(),
        forcePreview: !isTelegramBotConfigured()
