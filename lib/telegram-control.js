import { getAuthorizedStaff, listAuthorizedSubscribers, assignStaffRole } from './security/staff-access';
import { isFullCrmRole } from './security/access-policy';
...
import {
  getBOSEBotScenario
} from "./telegram/scenario-registry";
import { runBOSEBotScenarioAction } from "./telegram/scenario-service";
import { buildScenarioOrderTelegramReply } from "./telegram/scenario-order-bridge";
import { createProjectBotFlow, isProjectUpdate, projectMenuRows } from "./telegram/project-flow";
import { deliverProjectFile, listProjectManagers, projectAccess, withProjectChatLock } from "./telegram/project-service";
import { getProjectFile, listProjectFilesWithLegacy, saveProjectFile } from "./project-files";

const projectBotFlow = createProjectBotFlow({
  readState: readPendingRegistration,
  writeState: writePendingRegistration,
  clearState: clearPendingRegistration,
  getLead: getLeadBySlug,
  listLeads: getLeadsData,
  createLead: createTelegramClientLead,
  updateLead: updateLeadWorkflow,
  listFiles: async (id) => {
    const lead = await getLeadBySlug(id);
    return lead ? listProjectFilesWithLegacy(lead) : [];
  },
  getFile: getProjectFile,
  saveFile: saveProjectFile,
  managers: () => listProjectManagers(getTelegramSubscribers()),
  access: projectAccess,
  deliverFile: deliverProjectFile,
  send: (chatId, text, replyMarkup) => deliverTelegramMessage(chatId, text, { replyMarkup }),
  ack: (id) => acknowledgeTelegramCallback(id, "Открываю"),
  log: (error) => console.warn("Project bot action failed:", error.code || "project_action_error")
});

async function handleProjectBotUpdate(update) {
  const chatId = update.callback_query?.message?.chat?.id || update.message?.chat?.id;
  if (!chatId || !isProjectUpdate(update, readPendingRegistration(chatId))) return null;
  try {
    return await withProjectChatLock(String(chatId), async () => {
      await refreshTelegramRegistrationStateStore();
      return projectBotFlow(update, getPreferredTelegramSubscriber(chatId));
    });
  } catch (error) {
    console.warn("Project bot busy:", error.code || "project_state_error");
    return { ok: false, action: "project_busy", reply: await deliverTelegramMessage(chatId, "Проект сейчас обновляется. Повторите действие через несколько секунд.") };
  }
}
...
function applyVercelProtectionBypass(target) {
...
function buildAbsoluteAppUrl(path) {
  ...
  try {
    ...
    return applyVercelProtectionBypass(target.toString());
  } catch {
    return applyVercelProtectionBypass(`${baseUrl}${normalizedPath}`);
  }
}
...
async function trackTelegramBotEvent(eventName, payload = {}, chat = null, from = null) {
  const telegramUserId = from?.id ? String(from.id) : null;
...
function buildAbsoluteLeadUrl(slug, section = "summary") {
  ...
  return href ? buildAbsoluteAppUrl(href) : null;
}
...
async function loadStoredTelegramSubscribers() { return listAuthorizedSubscribers(); }
...
function getTelegramSubscribers() {
  ...
  for (const subscriber of getStoredTelegramSubscribers()) {
...
// Persistent staff shortcuts complement contextual inline buttons in project cards.
function buildStaffReplyKeyboard() {
  const appUrl = buildAbsoluteAppUrl("/");
  const appButton = /^https:\/\//i.test(appUrl)
    ? { text: "Открыть BOSE", web_app: { url: appUrl } }
    : { text: "Открыть BOSE", url: appUrl };

  return {
    keyboard: [
      [{ text: "Новая заявка" }, { text: "Проекты" }],
      [{ text: "Сегодня" }, { text: "Инструменты" }],
      [appButton]
    ],
...
function buildRegistrationRoleKeyboard() {
  return buildInlineKeyboard([
    ...
    [
      ...
      {
        text: "Дизайнер",
        callback_data: "reg|role|designer"
      }
    ],
    [
      {
        text: "Производство",
        callback_data: "reg|role|production"
      },
      {
        text: "Монтажник",
        callback_data: "reg|role|installer"
      }
    ]
  ]);
}
...
function buildOpenRouteButton(path, label) {
  const targetUrl = buildAbsoluteAppUrl(path);
...
function buildOpenMiniAppRouteButton(path, label = "Open Scenario Form") {
  const targetUrl = buildAbsoluteAppUrl(path);
...
function buildOpenAppButton(label = "Открыть BOSE") {
  const normalizedLabel = String(label || "").trim();
  label = /^open /i.test(normalizedLabel)
    ? normalizedLabel
    : /mini app/i.test(normalizedLabel)
      ? "Open Mini App"
      : "Open BOSE";
  const appUrl = buildAbsoluteAppUrl("/");
...
function withAppAccessReplyMarkup(replyMarkup, label = "Открыть BOSE") {
  // Reply keyboards are persistent staff controls. They cannot be combined
  // with inline keyboards, so keep them intact instead of replacing them.
...
function buildHelpText(subscriber) {
  return [
    "BOSE bot for clients, deals, tasks, and AI",
    "",
    subscriber
      ? `Current role: ${getTelegramRoleLabel(subscriber.role)}${subscriber.name ? ` (${subscriber.name})` : ""}`
      : "This chat is not registered yet.",
    "",
    "Commands:",
    "/register manager YourName",
    "/status",
    "/app",
    "/add",
    "/today",
    ...
    "/pilot",
    "",
    "Manager text prompts still work for summary, clients, deals, tasks, and AI."
  ].join("\n");

  return [
    "Бот BOSE для клиентов, сделок и задач",
    "",
    subscriber
    ...
    "/register дизайнер Имя",
    "/register производство Имя",
    "/register монтажник Имя",
    "/status",
    "/app",
    "/today",
    "/control",
...
function buildClientMenuKeyboard(lead = null) {
  ...
  if (lead?.slug) {
    const openOrderButton = buildOpenDealButton(lead.slug, "summary", "Open order");

    if (openOrderButton) {
      englishRows.push([openOrderButton]);
    }
  }

  return buildInlineKeyboard(englishRows);
...
function buildClientHelpText(lead = null) {
  return [
    "BOSE is a Telegram-first business workspace.",
    "",
    lead?.slug
      ? `Your order is linked: ${lead.name}${lead.orderNumber ? ` (${lead.orderNumber})` : ""}`
      : "Use this bot to leave a request, check order status, confirm a visit, or ask for an estimate.",
    "",
    "You can:",
    "- leave a request",
    "- check order status",
    ...
    "- contact a manager",
    "",
    "Tap a button below or type: request, status, visit, estimate, deposit."
  ].join("\n");

  return [
    "BOSE — Telegram-first business workspace",
    "",
    lead?.slug
...
function buildClientDemoTextV2() {
  return [
    "How to show BOSE in a demo:",
    "",
    "1. A client sends a request in Telegram.",
    "2. BOSE creates the record immediately.",
    "3. The manager sees the new work in the workspace.",
    "4. The team moves it to visit, estimate, and deposit.",
    "",
    "For the demo, open the three states below: new request, estimate, and production."
  ].join("\n");
...
function buildClientDemoReplyMarkupV2(lead = null) {
  const demoRowsV2 = [
    [buildOpenDealButton("l-201", "summary", "New request")],
    [buildOpenDealButton("l-202", "estimate", "Estimate and project")],
    [buildOpenDealButton("l-206", "production", "Deposit and production")]
  ].filter((row) => row.every(Boolean));
  const demoWorkboardButton = buildOpenRouteButton("/workboard", "Open workboard");
  const demoAppointmentsButton = buildOpenRouteButton("/appointments", "Open appointments");
  const demoPilotButton = buildOpenBotStartButton("pilot", "Request pilot launch");

  if (demoWorkboardButton) {
    demoRowsV2.push([demoWorkboardButton]);
  }

  if (demoAppointmentsButton) {
    demoRowsV2.push([demoAppointmentsButton]);
  }

  if (demoPilotButton) {
    demoRowsV2.push([demoPilotButton]);
  }

  const clientMenuRowsV2 = buildClientMenuKeyboard(lead)?.inline_keyboard || [];
  return buildInlineKeyboard([...clientMenuRowsV2, ...demoRowsV2]);
...
function resolveTelegramTextIntent(text, subscriber) {
  ...
  if (containsAny("app", "crm", "workspace", "мини апп", "mini app", "приложение", "панель")) {
    return {
  ...
  if (containsAny("clients", "client", "клиенты", "клиент")) {
    return {
      key: "manager_core_clients",
      ttlSeconds: 20
    };
  }

  if (containsAny("deals", "deal", "сделки", "сделка", "воронка")) {
    return {
      key: "manager_core_deals",
      ttlSeconds: 20
    };
  }

  if (containsAny("tasks", "task", "задачи", "задача")) {
    return {
      key: "manager_core_tasks",
      ttlSeconds: 20
    };
  }

  if (containsAny("ai", "ии", "assistant", "ассистент")) {
    return {
      key: "manager_core_ai",
      ttlSeconds: 20
    };
...
async function buildTelegramTextIntentMessages(intentKey, subscriber) {
  ...
  switch (intentKey) {
    ...
    case "manager_workspace":
      return buildCommandMessages("app", subscriber);
    case "manager_core_clients":
      return buildCommandMessages("clients", subscriber);
    case "manager_core_deals":
      return buildCommandMessages("deals", subscriber);
    case "manager_core_tasks":
      return buildCommandMessages("tasks", subscriber);
    case "manager_core_ai":
      return buildCommandMessages("ai", subscriber);
    default:
      return [];
  }
}
...
function getDashboardMetricValue(dashboard, label, fallback = "0") {
  const metric = Array.isArray(dashboard?.metrics)
    ? dashboard.metrics.find((item) => item?.label === label)
    : null;

  return metric?.value || String(fallback);
}

function buildKeyboardFromRows(rows = []) {
  return buildInlineKeyboard(
    rows.filter((row) => Array.isArray(row) && row.length && row.every(Boolean))
  );
}

function isSkipValue(value) {
  return ["skip", "no", "-", "later", "пропустить"].includes(
...
function normalizeScenarioValue(value) {
  ...
  return trimmed && !isSkipValue(trimmed) ? trimmed : null;
}

function isPendingScenarioFlow(flow) {
  return String(flow || "").startsWith("scenario_");
}

function getPendingScenario(chatId) {
  const pending = readPendingRegistration(chatId);
  return isPendingScenarioFlow(pending?.flow) ? pending : null;
}

function buildScenarioStartButton(scenarioId, label) {
  return {
    text: label,
    callback_data: `sc|start|${scenarioId}`
  };
}

function buildScenarioConfirmKeyboard(scenarioId) {
  return buildKeyboardFromRows([
    [
      {
        text: "Подтвердить",
        callback_data: `sc|confirm|${scenarioId}`
      },
      {
        text: "Отмена",
        callback_data: `sc|cancel|${scenarioId}`
      }
    ]
  ]);
}

function buildScenarioCancelKeyboard(scenarioId) {
  return buildKeyboardFromRows([
    [
      {
        text: "Отмена",
...
function buildScenarioHubReplyMarkup() {
  return buildKeyboardFromRows([
    ...projectMenuRows(),
    [buildScenarioStartButton("scenario_discovery", "Что автоматизировать?")],
    [
      buildScenarioStartButton("create_client", "Создать клиента"),
      buildScenarioStartButton("create_deal", "Создать сделку")
    ],
    [
      buildScenarioStartButton("create_task", "Создать задачу"),
      buildScenarioStartButton("followup_reminder", "Напоминание")
    ],
    [
      buildScenarioStartButton("ask_ai", "Спросить AI"),
      buildScenarioStartButton("daily_summary", "Сводка")
    ],
    [
      buildScenarioStartButton("customer_feedback", "Обратная связь"),
      buildScenarioStartButton("feature_request", "Предложить функцию")
    ],
    [
      buildScenarioStartButton("support_request", "Поддержка"),
      buildScenarioStartButton("file_document_intake", "Файл или документ")
    ]
  ]);
}

function buildScenarioHubText() {
  return [
    "Инструменты BOSE",
    "",
    "Выберите действие кнопкой ниже. Команды запоминать не нужно."
  ].join("\n");
}

function buildScenarioDiscoveryCategoryKeyboard() {
  const buttons = listScenarioDraftCategories().map((category) => ({
    text: getScenarioDraftCategoryLabel(category.id, "ru"),
    callback_data: `sc|category|scenario_discovery|${category.id}`
  }));
  const rows = [];
  ...
  rows.push([
    {
      text: "Cancel",
      callback_data: "sc|cancel|scenario_discovery"
    }
  ]);

  return buildKeyboardFromRows(rows);
}

function buildScenarioDiscoveryIntroText() {
  return [
    "What do you want to automate?",
    "",
    "Choose the area first, then describe how the process works now."
  ].join("\n");
}

function getScenarioDiscoveryProcessPrompt(categoryId) {
  ...
  return [
    ...
    "Example: A client writes in Telegram, a manager replies, and if the client stays silent for two days, BOSE should remind them."
  ].join("\n");
}

function getScenarioStepPrompt(scenarioId, step) {
  const prompts = {
    create_client: [
      "Send client name.",
      "Send client phone.",
    ...
    create_deal: [
      "Send client ID, phone, or exact client name.",
      "Send deal title.",
      "Send amount estimate or type skip."
    ],
    create_task: [
      "Send task title.",
      "Send client, deal, or lead reference, or type skip.",
      "Send deadline like 2026-06-27 15:00 or type skip.",
      "Add task description or type skip."
    ],
    followup_reminder: [
      "Send client, deal, or lead reference.",
      "Send follow-up date and time.",
      "Add follow-up note."
    ],
    scenario_discovery: ["Choose a category first.", "Describe how it works now."],
    ask_ai: ["Send your question for BOSE AI."],
    customer_feedback: ["Write what feels missing or inconvenient in BOSE."],
    feature_request: ["Write which feature or automation you want."],
    support_request: ["Describe the issue or support request."],
    file_document_intake: ["Send a document or photo with an optional caption."]
  };

  return prompts[scenarioId]?.[step] || "Send the next value.";
}

function buildScenarioDraftText(scenarioId, data = {}) {
  switch (scenarioId) {
    case "create_client":
      return [
        "Create Client",
        `Name: ${data.displayName || "Missing"}`,
    ...
    case "create_deal":
      return [
        "Create Deal",
        `Client: ${data.clientReference || "Missing"}`,
        `Title: ${data.title || "Missing"}`,
        `Amount: ${data.amountEstimate || "Not set"}`
      ].join("\n");
    case "create_task":
      return [
        "Create Task",
    ...
    case "followup_reminder":
      return [
        ...
        `When: ${data.scheduledAt || "Missing"}`,
        `Note: ${data.note || "Missing"}`
      ].join("\n");
    case "scenario_discovery":
      return [
        "Scenario Draft",
        `Category: ${getScenarioDraftCategoryLabel(data.category || "other", "en")}`,
        "",
    ...
    case "file_document_intake":
      return [
        "File Intake",
        `File: ${data.fileName || "Attachment"}`,
        `Caption: ${data.caption || "None"}`
      ].join("\n");
    default:
      return data.content || data.question || "Ready to submit.";
  }
}
...
async function startBotScenarioFlow(scenarioId, chat, from, subscriber = null) {
  if (scenarioId === "furniture_project") {
    return handleProjectBotUpdate({ message: { text: "/newrequest", chat, from } });
  }
  const scenario = getBOSEBotScenario(scenarioId);
  const publicScenarioIds = new Set([
    "scenario_discovery",
    "ask_ai",
    "customer_feedback",
    "feature_request",
  ...
  if (!scenario) {
    return {
      ...
      reply: await deliverTelegramMessage(chat?.id, "Scenario not found.", {
        forcePreview: !isTelegramBotConfigured()
      })
    };
  }

  if (!subscriber && !publicScenarioIds.has(scenario.id)) {
    return {
      ok: false,
      action: "scenario_requires_registration",
      reply: await deliverTelegramMessage(
        chat?.id,
        "Register as a team member first with /register manager YourName.",
        {
          forcePreview: !isTelegramBotConfigured()
        }
      )
  ...
  if (scenario.id === "daily_summary") {
    const result = await runBOSEBotScenarioAction({
      scenarioId: "daily_summary",
      actor: {
        telegramUserId: from?.id ? String(from.id) : null,
        chatId: chat?.id ? String(chat.id) : null,
        username: from?.username || null,
        firstName: subscriber?.name || from?.first_name || null,
        languageCode: from?.language_code || null,
    ...
    const text = [
      "Daily Summary",
      result?.data?.dailyDigest || "No summary available.",
      result?.data?.overdueTasks || "",
      ...(Array.isArray(result?.data?.managerRecommendations)
        ? result.data.managerRecommendations.slice(0, 3).map((item) => `- ${item}`)
        : [])
    ]
      .filter(Boolean)
      .join("\n\n");
    ...
    return {
      ok: true,
      action: "scenario_daily_summary",
      reply: await deliverTelegramMessage(chat?.id, text, {
        replyMarkup: buildKeyboardFromRows([[buildOpenRouteButton("/dashboard", "Open Dashboard")]]),
        forcePreview: !isTelegramBotConfigured()
      })
    };
  }

  if (scenario.id === "scenario_discovery") {
    await writePendingRegistration(chat?.id, {
      flow: `scenario_${scenario.id}`,
      scenarioId: scenario.id,
      stepIndex: 0,
      draft: {},
      startedAt: new Date().toISOString(),
      subscriberRole: subscriber?.role || null,
      telegramUserId: from?.id ? String(from.id) : null,
      username: from?.username || null
    });
    await trackTelegramBotEvent(
    ...
    return {
      ...
      action: "scenario_start",
      scenarioId: scenario.id,
      reply: await deliverTelegramMessage(chat?.id, buildScenarioDiscoveryIntroText(), {
        replyMarkup: buildScenarioDiscoveryCategoryKeyboard(),
        forcePreview: !isTelegramBotConfigured()
      })
    };
  }
  ...
  return {
    ...
    action: "scenario_start",
    scenarioId: scenario.id,
    reply: await deliverTelegramMessage(chat?.id, getScenarioStepPrompt(scenario.id, 0), {
      replyMarkup: buildScenarioCancelKeyboard(scenario.id),
      forcePreview: !isTelegramBotConfigured()
    })
  };
}

async function finalizeBotScenarioFlow(pendingState, chat, from, subscriber = null) {
  const scenarioId = pendingState?.scenarioId;
  const result = await runBOSEBotScenarioAction({
    scenarioId,
    data: pendingState?.draft || {},
    actor: {
      telegramUserId: from?.id ? String(from.id) : pendingState?.telegramUserId || null,
      chatId: chat?.id ? String(chat.id) : null,
      username: from?.username || pendingState?.username || null,
      firstName: subscriber?.name || from?.first_name || null,
      languageCode: from?.language_code || null,
      role: subscriber?.role || pendingState?.subscriberRole || null
    }
  });
  ...
  if (!result?.ok) {
    return {
      ok: false,
      action: "scenario_finalize",
      scenarioId,
      result,
      followupReply: await deliverTelegramMessage(
        chat?.id,
        result?.message || "Scenario failed.",
        { forcePreview: !isTelegramBotConfigured() }
      )
    };
  }

  let text = result?.message || "Scenario completed.";
  let replyMarkup = undefined;

  if (scenarioId === "scenario_discovery") {
    const draft = result?.draft || {};
    text = [
      result.created === false
        ? "BOSE already has this scenario draft."
        : "Done. BOSE drafted the scenario.",
      draft.aiSummary ? `BOSE understood: ${draft.aiSummary}` : null,
      draft.suggestedTrigger ? `First step to automate: ${draft.suggestedTrigger}` : null,
      "Next: the request is saved for owner review inside BOSE."
    ]
      .filter(Boolean)
      .join("\n\n");
    replyMarkup = buildKeyboardFromRows([
      [buildOpenRouteButton("/demo", "Open Demo")],
      [buildOpenRouteButton("/dashboard", "Open Workspace")]
    ]);
  } else if (scenarioId === "ask_ai") {
    const aiData = result?.data || {};
    text = [
      "BOSE AI",
      aiData.summary || "No summary available.",
      aiData.nextAction ? `Next: ${aiData.nextAction}` : null,
      aiData.suggestedScenario ? `Suggested scenario: ${aiData.suggestedScenario}` : null,
      aiData.draftReply ? `Draft: ${aiData.draftReply}` : null
    ]
      .filter(Boolean)
      .join("\n\n");
    replyMarkup = buildKeyboardFromRows([
      aiData.suggestedScenario
        ? [buildScenarioStartButton(aiData.suggestedScenario, "Run suggested scenario")]
        : null,
      [buildOpenRouteButton("/ai", "Open AI")],
      [buildOpenRouteButton("/dashboard", "Open Dashboard")]
    ]);
  } else if (scenarioId === "create_client") {
    text = `Client saved.\n\n${result.client?.displayName || "Client"}${result.client?.primaryPhone ? ` - ${result.client.primaryPhone}` : ""}`;
    replyMarkup = buildKeyboardFromRows([
      result.client?.id ? [buildOpenRouteButton(`/clients/${result.client.id}`, "Open Client")] : null,
      [buildScenarioStartButton("create_deal", "Create Deal")],
      [buildOpenRouteButton("/clients", "Open Clients")]
    ]);
  } else if (scenarioId === "create_deal") {
    text = `Deal saved.\n\n${result.deal?.title || "Deal"}${result.client?.displayName ? ` for ${result.client.displayName}` : ""}`;
    replyMarkup = buildKeyboardFromRows([
      result.deal?.id ? [buildOpenRouteButton(`/deals/${result.deal.id}`, "Open Deal")] : null,
      [buildScenarioStartButton("create_task", "Create Task")],
      [buildOpenRouteButton("/deals", "Open Deals")]
    ]);
  } else if (scenarioId === "create_task") {
    replyMarkup = buildKeyboardFromRows([
      [buildOpenRouteButton("/tasks", "Open Tasks")],
      [buildScenarioStartButton("followup_reminder", "Create Follow-up")]
    ]);
  } else if (scenarioId === "followup_reminder") {
    replyMarkup = buildKeyboardFromRows([
      [buildOpenRouteButton("/tasks", "Open Tasks")],
      [buildOpenRouteButton("/dashboard", "Open Dashboard")]
    ]);
  } else if (
    ["customer_feedback", "feature_request", "support_request", "file_document_intake"].includes(
      scenarioId
    )
  ) {
    text = [
      result?.message || "Request captured.",
      result?.aiSummary?.summary ? `AI summary: ${result.aiSummary.summary}` : null,
      result?.aiSummary?.nextAction ? `AI next step: ${result.aiSummary.nextAction}` : null
    ]
      .filter(Boolean)
      .join("\n\n");
    replyMarkup = buildKeyboardFromRows([
  ...
  return {
    ...
    scenarioId,
    result,
    followupReply: await deliverTelegramMessage(chat?.id, text, {
      replyMarkup,
      forcePreview: !isTelegramBotConfigured()
    })
  };
...
async function handleBotScenarioFlow(messageText, chat, from, subscriber, pendingState) {
  const scenarioId = pendingState?.scenarioId;
  const draft = { ...(pendingState?.draft || {}) };
  const stepIndex = Number(pendingState?.stepIndex || 0);
  ...
  switch (scenarioId) {
    case "scenario_discovery":
      if (stepIndex === 0) {
        const category = normalizeScenarioDraftCategory(messageText, null);

        if (!category) {
          return {
            ok: true,
            action: "scenario_category_required",
            scenarioId,
            reply: await deliverTelegramMessage(
              chat?.id,
              "Choose one of the categories first.",
              {
                replyMarkup: buildScenarioDiscoveryCategoryKeyboard(),
                forcePreview: !isTelegramBotConfigured()
              }
        ...
        return {
          ...
          reply: await deliverTelegramMessage(
            chat?.id,
            getScenarioDiscoveryProcessPrompt(category),
            {
              replyMarkup: buildScenarioCancelKeyboard(scenarioId),
              forcePreview: !isTelegramBotConfigured()
            }
          )
        };
      ...
      await clearPendingRegistration(chat?.id);
      return finalizeBotScenarioFlow(
        {
          ...pendingState,
          draft
        },
    ...
    case "create_client":
      if (stepIndex === 0) {
        draft.displayName = messageText;
      } else if (stepIndex === 1) {
        draft.phone = messageText;
      } else {
        draft.note = normalizeScenarioValue(messageText);
      }
      break;
    case "create_deal":
      if (stepIndex === 0) {
        draft.clientReference = messageText;
      } else if (stepIndex === 1) {
        draft.title = messageText;
      } else {
        draft.amountEstimate = normalizeScenarioValue(messageText);
      }
      break;
    case "create_task":
      if (stepIndex === 0) {
        ...
        draft.reference = normalizeScenarioValue(messageText);
      } else if (stepIndex === 2) {
        const parsedDate = parseBusinessDate(messageText);
        draft.deadline = parsedDate ? parsedDate.toISOString() : normalizeScenarioValue(messageText);
      } else {
        draft.description = normalizeScenarioValue(messageText);
      }
      break;
    case "followup_reminder":
      if (stepIndex === 0) {
        draft.reference = messageText;
      } else if (stepIndex === 1) {
        const parsedDate = parseBusinessDate(messageText);
        draft.scheduledAt = parsedDate ? parsedDate.toISOString() : messageText;
      } else {
        draft.note = messageText;
      }
      break;
    case "ask_ai":
      draft.question = messageText;
      await clearPendingRegistration(chat?.id);
      return finalizeBotScenarioFlow(
        {
          ...pendingState,
    ...
    case "file_document_intake":
      return {
        ok: true,
        action: "scenario_wait_file",
        scenarioId,
        reply: await deliverTelegramMessage(chat?.id, "Send a document or photo for file intake.", {
          forcePreview: !isTelegramBotConfigured()
        })
      };
    default:
  ...
  const lastStepIndex =
    getBOSEBotScenario(scenarioId)?.steps?.length > 0
      ? getBOSEBotScenario(scenarioId).steps.length - 1
      : 0;
  ...
  if (nextStepIndex > lastStepIndex) {
    ...
    return {
      ok: true,
      action: "scenario_ready",
      scenarioId,
      reply: await deliverTelegramMessage(chat?.id, buildScenarioDraftText(scenarioId, draft), {
        replyMarkup: buildScenarioConfirmKeyboard(scenarioId),
        forcePreview: !isTelegramBotConfigured()
      })
    };
  }
  ...
  return {
    ...
    action: "scenario_step",
    scenarioId,
    reply: await deliverTelegramMessage(chat?.id, getScenarioStepPrompt(scenarioId, nextStepIndex), {
      replyMarkup: buildScenarioCancelKeyboard(scenarioId),
      forcePreview: !isTelegramBotConfigured()
    })
  };
...
async function handleBotScenarioAttachment(update, pendingState, subscriber = null) {
  const scenarioId = pendingState?.scenarioId;

  if (scenarioId !== "file_document_intake") {
    return null;
  }

  const document = update?.message?.document || null;
  const photo = Array.isArray(update?.message?.photo)
    ? update.message.photo[update.message.photo.length - 1] || null
    : null;
  ...
  const draft = {
    ...(pendingState?.draft || {}),
    fileId: document?.file_id || photo?.file_id || null,
    fileName: document?.file_name || (photo ? "telegram-photo.jpg" : "attachment"),
    mimeType: document?.mime_type || (photo ? "image/jpeg" : null),
    caption: normalizeScenarioValue(update?.message?.caption)
  };

  await writePendingRegistration(update?.message?.chat?.id, {
    ...pendingState,
    draft,
    stepIndex: 1
  });
  ...
  return {
    ok: true,
    action: "scenario_file_ready",
    scenarioId,
    reply: await deliverTelegramMessage(
      update?.message?.chat?.id,
      buildScenarioDraftText(scenarioId, draft),
      {
        replyMarkup: buildScenarioConfirmKeyboard(scenarioId),
        forcePreview: !isTelegramBotConfigured()
      }
...
async function buildCommandMessages(command, subscriber) {
  ...
  switch (command) {
    ...
    case "menu":
      return [
        {
          text: buildScenarioHubText(),
          replyMarkup: buildScenarioHubReplyMarkup()
        }
      ];
    ...
    case "workspace":
      return [
        {
          text: [
            "BOSE уже готов к работе из Telegram.",
            "Открывай рабочее пространство, клиентов, сделки, задачи и AI из кнопок ниже."
          ].join("\n\n"),
          replyMarkup: buildKeyboardFromRows([
            [buildOpenRouteButton("/dashboard", "Открыть BOSE")],
            [
              buildOpenRouteButton("/clients", "Клиенты"),
              buildOpenRouteButton("/deals", "Сделки")
            ],
            [
              buildOpenRouteButton("/tasks", "Задачи"),
              buildOpenRouteButton("/ai", "AI")
            ],
            [
              buildScenarioStartButton("create_client", "Create Client"),
              buildScenarioStartButton("create_task", "Create Task")
            ],
            [buildScenarioStartButton("ask_ai", "Ask AI")]
          ])
        }
      ];
    ...
    case "clients": {
      ...
      const clients = Array.isArray(dashboard?.queues?.clients) ? dashboard.queues.clients.slice(0, 4) : [];

      return [
        {
          text: [
            `Клиентов в BOSE: ${getDashboardMetricValue(dashboard, "Clients", clients.length)}`,
            ...clients.map((item) => `• ${item.displayName} — ${item.primaryPhone || "No phone"} — ${item.status}`)
          ].join("\n"),
          replyMarkup: buildKeyboardFromRows([
            ...clients.map((item) => [buildOpenRouteButton(`/clients/${item.id}`, item.displayName)]),
            [buildOpenRouteButton("/clients", "Открыть клиентов")]
          ])
        }
      ];
    }
    ...
    case "deals": {
      const dashboard = await getBOSEDashboardData();
      const deals = Array.isArray(dashboard?.queues?.deals) ? dashboard.queues.deals.slice(0, 4) : [];

      return [
        {
          text: [
            `Сделок в BOSE: ${getDashboardMetricValue(dashboard, "Deals", deals.length)}`,
            ...deals.map((item) => `• ${item.title} — ${item.stageKey} — ${item.client?.displayName || "No client"}`)
          ].join("\n"),
          replyMarkup: buildKeyboardFromRows([
            ...deals.map((item) => [buildOpenRouteButton(`/deals/${item.id}`, item.title)]),
            [buildOpenRouteButton("/deals", "Открыть сделки")]
          ])
        }
      ];
    }
    ...
    case "tasks": {
      const dashboard = await getBOSEDashboardData();
      const tasks = Array.isArray(dashboard?.queues?.tasks) ? dashboard.queues.tasks.slice(0, 4) : [];

      return [
        {
          text: [
            `Открытых задач: ${getDashboardMetricValue(dashboard, "Tasks", tasks.length)}`,
            ...tasks.map((item) => `• ${item.title} — ${item.owner || item.ownerName || "Unassigned"}`)
          ].join("\n"),
          replyMarkup: buildKeyboardFromRows([
            ...tasks
              .filter((item) => item?.slug)
              .map((item) => [buildOpenDealButton(item.slug, "summary", item.title)]),
            [buildOpenRouteButton("/tasks", "Открыть задачи")]
          ])
        }
      ];
    }
    ...
    case "ai": {
      const summary = await getAICRMSummary();
      const recommendations = Array.isArray(summary?.data?.managerRecommendations)
        ? summary.data.managerRecommendations.slice(0, 3)
        : [];
      ...
      return [
        {
          text: [
            "AI CRM Assistant",
            summary?.data?.dailyDigest || "Сводка пока не готова.",
            summary?.data?.overdueTasks || "Просроченных задач сейчас нет.",
            ...recommendations.map((item) => `• ${item}`)
          ].join("\n"),
          replyMarkup: buildKeyboardFromRows([
            [buildOpenRouteButton("/ai", "Открыть AI")],
            [buildOpenRouteButton("/dashboard", "Открыть панель")]
          ])
        }
      ];
    }
      ...
      return [
        {
          text: [
            "BOSE Daily Summary",
            summary?.data?.dailyDigest || "No summary available.",
            summary?.data?.overdueTasks || "",
            summary?.data?.workloadSummary || "",
            ...recommendations.map((item) => `- ${item}`)
          ]
            .filter(Boolean)
            .join("\n\n"),
...
export async function handleTelegramCommand(text, from, chat) {
  ...
  const projectResult = await handleProjectBotUpdate({ message: { text, from, chat } });
  ...
  const staff = await getAuthorizedStaff(from?.id);
  if (staff && (chat?.type !== 'private' || String(chat?.id) !== String(from?.id))) return {ok:false,action:'private_chat_required'};
  if (command === 'staff_role') {
    if (staff?.role !== 'owner') return {ok:false,action:'staff_role_denied',reply:await deliverTelegramMessage(chat?.id,'Роли назначает только владелец.')};
    const [telegramUserId,role,...nameParts] = rest;
    try { await assignStaffRole(String(from.id),{telegramUserId,role,name:nameParts.join(' ')}); return {ok:true,reply:await deliverTelegramMessage(chat.id,'Роль назначена. Старые сессии сотрудника завершены. Сотруднику нужно открыть /menu.')}; }
    catch(error) { return {ok:false,reply:await deliverTelegramMessage(chat.id,error.message)}; }
  }
  if (['start','help','menu','add'].includes(command) && staff) return {ok:true,action:'staff_menu',reply:await deliverTelegramMessage(chat.id,'Рабочее меню. Роль: '+getTelegramRoleLabel(({owner:'director',operator:'measurer'}[staff.role]||staff.role))+(staff.role==='owner'?'\nНазначить роль: /staff_role TelegramID роль Имя':''),{replyMarkup:staffMenuMarkup({coreRole:staff.role})})};
  if (staff && !isFullCrmRole(staff.role) && !['start','help','menu','add','app','workspace','status','register','cancel'].includes(command)) return {ok:false,action:'project_only',reply:await deliverTelegramMessage(chat.id,'Доступны проекты, фото и материалы.',{replyMarkup:staffMenuMarkup({coreRole:staff.role})})};
  if (!staff && !['start','help','register','request','order','estimate','deposit','manager','demo','pilot','status','cancel'].includes(command)) return {ok:false,action:'staff_required',reply:await deliverTelegramMessage(chat?.id,'Роль назначает руководитель. Ваш Telegram ID: '+String(from?.id))};
  if (staff && !isFullCrmRole(staff.role) && ['app','workspace'].includes(command)) return {ok:true,reply:await deliverTelegramMessage(chat.id,'Открыть проекты',{replyMarkup:{inline_keyboard:[[{text:'Проекты',web_app:{url:buildAbsoluteAppUrl('/projects')}}]]}})};
  ...
  if (command === "start" && startPayload) {
    ...
    if (startPayload === "scenario") {
      return handleTelegramCommand("/scenario", from, chat);
    }
  ...
  if (command === "start" || command === "help") {
    const reply = currentSubscriber
      ? await deliverTelegramMessage(chat?.id, buildHelpText(currentSubscriber), {
          replyMarkup: buildStaffReplyKeyboard(),
          forcePreview: !isTelegramBotConfigured()
        })
  ...
  if (command === "demo") {
    return startBotScenarioFlow("scenario_discovery", chat, from, currentSubscriber);
  }

  if (command === "scenario") {
    const launch = buildScenarioOrderTelegramReply();
    const reply = await deliverTelegramMessage(
      chat?.id,
      launch.text,
      {
        replyMarkup: buildKeyboardFromRows([
          [buildOpenMiniAppRouteButton(launch.path, launch.buttonLabel)]
        ]),
        forcePreview: !isTelegramBotConfigured(),
        appButtonLabel: "Open Mini App"
      }
    );

    return {
      ok: true,
      action: "scenario_order",
      reply
    };
  }
  ...
  const scenarioCommandMap = {
    ...
    file: "file_document_intake",
    summary: "daily_summary"
  };
  const scenario = getBOSEBotScenario(scenarioCommandMap[command] || command);
  ...
  if (scenario) {
    ...
    if (!currentSubscriber && !publicScenarioIds.has(scenario.id)) {
      return {
        ok: false,
        action: "scenario_requires_registration",
        reply: await deliverTelegramMessage(
    ...
    return startBotScenarioFlow(scenario.id, chat, from, currentSubscriber);
  }
  ...
  if (command === "status") {
    ...
    const englishStatusReply = [
      "Chat is linked.",
      `Role: ${getTelegramRoleLabel(currentSubscriber.role)}`,
      currentSubscriber.name ? `Name in system: ${currentSubscriber.name}` : null
    ]
      .filter(Boolean)
      .join("\n");
    const statusReply = await deliverTelegramMessage(chat?.id, englishStatusReply, {
      forcePreview: !isTelegramBotConfigured()
    });
  ...
  if (command === 'register') {
    await clearPendingRegistration(chat?.id);
    return {ok:true,action:'register',reply:await deliverTelegramMessage(chat?.id,currentSubscriber?'Ваша роль назначена руководителем. Откройте /menu.':'Роль назначает руководитель. Ваш Telegram ID: '+String(from?.id),{replyMarkup:staffMenuMarkup(currentSubscriber)})};
  }
...
export async function handleTelegramCallbackQuery(callbackQuery) {
  ...
  const projectResult = await handleProjectBotUpdate({ callback_query: callbackQuery });
  if (projectResult) return projectResult;

  const data = String(callbackQuery?.data || "");
  const [kind, action, firstArg, secondArg] = data.split("|");
  let result = null;
  const callbackStaff = await getAuthorizedStaff(callbackQuery?.from?.id);
  if ((callbackStaff && !isFullCrmRole(callbackStaff.role) && kind !== 'reg') || (kind !== 'cl' && kind !== 'reg' && (!callbackStaff || !isFullCrmRole(callbackStaff.role) || callbackQuery?.message?.chat?.type !== 'private' || String(callbackQuery.message.chat.id)!==String(callbackQuery.from.id)))) {
    await acknowledgeTelegramCallback(callbackQuery?.id,'Доступ запрещён. Откройте /menu.'); return {ok:false,action:'callback_denied'};
  }

  if (kind === 'reg') {
    await clearPendingRegistration(callbackQuery?.message?.chat?.id);
    await acknowledgeTelegramCallback(callbackQuery?.id,'Роли назначает руководитель. Откройте /menu.');
    return {ok:false,action:'registration_role_denied'};
  }

  if (kind === "sc") {
    const chat = callbackQuery?.message?.chat;
    const from = callbackQuery?.from;
    const subscriber = getPreferredTelegramSubscriber(chat?.id);
    const pendingScenario = getPendingScenario(chat?.id);

    if (action === "start" && firstArg) {
      await acknowledgeTelegramCallback(callbackQuery?.id, "Opening scenario");
      return startBotScenarioFlow(firstArg, chat, from, subscriber);
    }

    if (action === "cancel") {
      await clearPendingRegistration(chat?.id);
      await acknowledgeTelegramCallback(callbackQuery?.id, "Scenario cancelled");

      return {
        ok: true,
        action: "scenario_cancel",
        followupReply: await deliverTelegramMessage(chat?.id, "Scenario cancelled.", {
          replyMarkup: buildScenarioHubReplyMarkup(),
          forcePreview: !isTelegramBotConfigured()
        })
      };
    ...
    if (
      action === "category" &&
      firstArg === "scenario_discovery" &&
      pendingScenario?.scenarioId === "scenario_discovery"
    ) {
      const category = normalizeScenarioDraftCategory(secondArg, null);

      if (!category) {
        await acknowledgeTelegramCallback(callbackQuery?.id, "Category not found");
        return {
          ok: false,
          action: "scenario_category",
          result: {
            ok: false,
      ...
      await acknowledgeTelegramCallback(callbackQuery?.id, "Category selected");
      ...
      return {
        ...
        followupReply: await deliverTelegramMessage(
          ...
          getScenarioDiscoveryProcessPrompt(category),
          {
            replyMarkup: buildScenarioCancelKeyboard("scenario_discovery"),
            forcePreview: !isTelegramBotConfigured()
          }
        )
      };
    ...
    if (action === "confirm" && pendingScenario?.scenarioId === firstArg) {
      await acknowledgeTelegramCallback(callbackQuery?.id, "Creating");
      return finalizeBotScenarioFlow(pendingScenario, chat, from, subscriber);
    }
  }
  ...
  if (kind === "cl") {
    ...
    if (action === "apconfirm" && firstArg) {
      const ownLead = await findLeadForClientChat(chat,from);
      if (!(ownLead?.appointments || []).some(item=>item.id===firstArg)) return {ok:false,action:'appointment_access_denied'};
...
export async function handleTelegramWebhookUpdate(update) {
  ...
  if (staffMenuCommand) {
    return handleTelegramCommand(staffMenuCommand, update.message.from, update.message.chat);
  }

  const projectResult = await handleProjectBotUpdate(update);
  if (projectResult) return projectResult;

  const chatId = update?.message?.chat?.id;
  const messageText = String(update?.message?.text || "").trim();
  const pendingRegistration = readPendingRegistration(chatId);
  const pendingScenario = getPendingScenario(chatId);
  const activeStaff = await getAuthorizedStaff(update?.message?.from?.id);
  if (pendingScenario && !isFullCrmRole(activeStaff?.role)) {
    await clearPendingRegistration(chatId); return {ok:false,action:'scenario_access_denied',reply:await deliverTelegramMessage(chatId,'Доступ к этому сценарию закрыт. Откройте /menu.')};
  }
  const managerSubscriber = getPreferredTelegramSubscriber(chatId,'manager');
  const currentSubscriber = getPreferredTelegramSubscriber(chatId);

  if (pendingScenario && (update?.message?.document || update?.message?.photo)) {
    const attachmentResult = await handleBotScenarioAttachment(
      update,
      pendingScenario,
      currentSubscriber
    );
  ...
  if (messageText && pendingScenario) {
    const scenarioResult = await handleBotScenarioFlow(
      messageText,
      update.message.chat,
      update.message.from,
      currentSubscriber,
  ...
  if (messageText && activeStaff && !isFullCrmRole(activeStaff.role)) return {ok:true,reply:await deliverTelegramMessage(chatId,'Откройте проекты, чтобы добавить фото и файлы.',{replyMarkup:staffMenuMarkup({coreRole:activeStaff.role})})};

  if (messageText && managerSubscriber && isFullCrmRole(activeStaff?.role)) {
    ...
    const scenarioIntent = getBOSEBotScenario(messageText);

    if (scenarioIntent) {
      return startBotScenarioFlow(
        scenarioIntent.id,
        update.message.chat,
        update.message.from,
        managerSubscriber
  ...
  if (pendingRegistration?.flow === 'register') {
    await clearPendingRegistration(chatId);
    return {ok:false,action:'registration_role_denied',reply:await deliverTelegramMessage(chatId,'Роли назначает руководитель. Ваш ID: '+String(update.message.from?.id))};
  }
...
function staffMenuMarkup(subscriber) {
  const role=subscriber?.coreRole || mapTelegramRoleToCoreRole(subscriber?.role,null);
  if(!role)return {remove_keyboard:true};
  const keyboard=[[{text:'Проекты'}],[{text:'Меню'}]];
  if(isFullCrmRole(role))keyboard.unshift([{text:'Новая заявка'},{text:'Сегодня'}]);
