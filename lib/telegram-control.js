...
function buildManagerAppointmentsTextMessage(data, subscriber) {
  ...
  if (!ownTodayAppointments.length) {
    return {
      text: "На сегодня по вашим сделкам замеров и консультаций нет."
    };
  }

  const visible = ownTodayAppointments.slice(0, 4);
  const lines = [
    `На сегодня у вас ${ownTodayAppointments.length} замер(а) или консультации:`,
    "",
    ...visible.map(
      (item, index) =>
  ...
  if (ownTodayAppointments.length > visible.length) {
    lines.push("", `Ещё ${ownTodayAppointments.length - visible.length} смотрите в /appointments.`);
  }
...
function buildManagerConsultationTextMessage(data, subscriber) {
  const candidates = buildManagerConsultationCandidates(data, subscriber);

  if (!candidates.length) {
    return {
      text: "Сейчас по вашим сделкам нет клиентов, которым срочно нужен контакт или консультация."
    };
  }
...
function resolveTelegramTextIntent(text, subscriber) {
  ...
  const asksConsultationQueue =
    ...
    ((normalized.includes("проконсульт") ||
      normalized.includes("консультац") ||
      normalized.includes("позвон") ||
      normalized.includes("созвон") ||
      normalized.includes("связат")) &&
...
export async function handleTelegramWebhookUpdate(update) {
  ...
  if (update?.message?.text) {
    const currentSubscriber =
      getTelegramSubscribers().find(
        (item) => String(item.chatId) === String(update?.message?.chat?.id)
      ) || null;
