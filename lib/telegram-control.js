...
function buildScenarioHubReplyMarkup() {
  return buildKeyboardFromRows([
    ...projectMenuRows(),
    [buildScenarioStartButton("scenario_discovery", "What to automate?")],
    [
    ...
    [
      ...
      buildScenarioStartButton("followup_reminder", "Follow-up")
    ],
    [
      buildScenarioStartButton("ask_ai", "Ask AI"),
      buildScenarioStartButton("daily_summary", "Daily Summary")
    ],
    [
      buildScenarioStartButton("customer_feedback", "Feedback"),
      buildScenarioStartButton("feature_request", "Feature Request")
    ],
    [
      buildScenarioStartButton("support_request", "Support"),
      buildScenarioStartButton("file_document_intake", "File Intake")
    ]
  ]);
}

function buildScenarioHubText() {
  const launchScenarios = listBOSEBotLaunchScenarios().map(
    (scenario) => `/${scenario.command} - ${scenario.resultMessage}`
  );

  return [
    "BOSE bot actions",
    "",
    "Choose a business scenario or use a command.",
    ...launchScenarios
  ].join("\n");
}
...
function buildScenarioDiscoveryCategoryKeyboard() {
  const buttons = listScenarioDraftCategories().map((category) => ({
    text: getScenarioDraftCategoryLabel(category.id, "en"),
