...
export function projectAccess(userId) {
  const editors = parseIds(process.env.TELEGRAM_PROJECT_EDITOR_IDS || "", process.env.BOSE_OWNER_TELEGRAM_IDS || "", process.env.TELEGRAM_DIRECTOR_CHAT_IDS || "", process.env.TELEGRAM_MANAGER_CHAT_IDS || "", process.env.TELEGRAM_DESIGNER_CHAT_IDS || "");
  const viewers = parseIds(process.env.TELEGRAM_PROJECT_VIEWER_IDS || "", process.env.TELEGRAM_MEASURER_CHAT_IDS || "", process.env.TELEGRAM_PRODUCTION_CHAT_IDS || "", process.env.TELEGRAM_INSTALLER_CHAT_IDS || "");
  const photoContributors = parseIds(process.env.TELEGRAM_INSTALLER_CHAT_IDS || "");
  ...
  return {
    read: write || viewers.has(String(userId)),
    write,
    addPhoto: write || photoContributors.has(String(userId))
  };
}
