...
export function createProjectBotFlow(deps) {
  return async function handle(update, subscriber) {
    ...
    const card = async (id, prefix = "") => {
      ...
      if (canEdit) rows.push([button("Добавить файл", "upload", id), button("Добавить фото", "photo", id)], [button("Обновить проект", "update", id)]);
    ...
    try {
      ...
      if (["upload", "photo", "replace", "update"].includes(action)) {
        ...
        if (!await deps.getLead(id)) return finish("Проект не найден.");
        await setState({ mode: action === "update" ? "update" : "upload", leadId: id, token: randomUUID().slice(0, 8), groupKey: previous?.groupKey || null, expectedCurrentId: previous?.id || null, photoOnly: action === "photo" });
        ...
        return finish(previous ? `Отправьте новую версию: ${previous.fileName}. Старую сохраним в истории.` : `Отправьте ${action === "photo" ? "фото" : "PDF, фото, документ, чертёж или архив"} (до 20 МБ). Один файл за раз.`, [[button("Отмена", "cancel", state.token)], back(id)]);
      }
      if (!owned) return finish("Сценарий завершён или устарел. Откройте нужный проект.");
      if (callback && !["answer", "confirm", "edit", "comment", "savefile", "separate"].includes(action)) return finish("Откройте меню проекта заново.");
