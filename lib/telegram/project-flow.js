...
export function createProjectBotFlow(deps) {
  return async function handle(update, subscriber) {
    ...
    const employee = chat?.type === "private" && subscriber &&
      ["director", "manager", "measurer", "designer", "production", "installer"].includes(subscriber.role) &&
    ...
    const projectPermissions = deps.access ? deps.access(from.id) : null;
    ...
    try {
      ...
      const canContinuePhotoUpload = owned && canAddPhoto && state?.mode === "batchupload" && state.photoOnly;
      if (!canEdit && !(canAddPhoto && action === "photo") && !canContinuePhotoUpload) {
