import { canReadProjectFile, isFullCrmRole, projectDto } from '../security/access-policy.js';
...
export function createProjectBotFlow(deps) {
  return async function handle(update, subscriber) {
    ...
    const projectPermissions = deps.access ? await deps.access(from?.id) : null;
    const employee = chat?.type === "private" && subscriber &&
      ["director", "manager", "measurer", "designer", "production", "installer", "workshop_head", "production_manager"].includes(subscriber.role) &&
      String(subscriber.telegramUserId || subscriber.chatId) === String(from?.id) && String(subscriber.chatId) === String(chat?.id) &&
      (!deps.access || projectPermissions?.read);
    ...
    const canEdit = projectPermissions ? projectPermissions.write : ["director", "manager", "designer"].includes(subscriber.role);
    const canAddPhoto = projectPermissions ? projectPermissions.addPhoto : canEdit;
    const canAddFiles = projectPermissions ? projectPermissions.addFiles : canEdit;
    const coreRole = projectPermissions?.role || ({director:"owner",measurer:"operator"}[subscriber.role] || subscriber.role);
    const visibleFile = file => canReadProjectFile(coreRole,file);
    ...
    const list = async (group, page = 0) => {
      ...
      const rows = all.slice(start, start + 8).map((lead) => [button(`${short(lead.id, 8)} · ${short(isFullCrmRole(coreRole) ? lead.name : projectDto(lead,coreRole).title, 35)} · ${short(lead.product, 25)}`, "open", lead.slug)]);
    ...
    const card = async (id, prefix = "") => {
      ...
      else if (canAddFiles) rows.push([button("Добавить файлы", "upload", id), button("Добавить фото", "photo", id)]);
      else if (canAddPhoto) rows.push([button("Добавить фото", "photo", id)]);
      rows.push([button("Назад к списку", "list", getProjectGroup(lead))]);
      return finish([
        prefix, `Проект ${short(lead.orderNumber || lead.id, 35)}`, isFullCrmRole(coreRole) ? `Клиент: ${short(lead.name)}` : null,
        `Заказ: ${short(lead.product)}`, projectDto(lead,coreRole).address ? `Адрес: ${short(projectDto(lead,coreRole).address)}` : null, isFullCrmRole(coreRole) ? `Менеджер: ${short(lead.manager)}` : null,
    ...
    const files = async (id, mode, page = 0) => {
      ...
      const all = (await deps.listFiles(id)).filter(visibleFile);
    ...
    const deliver = async (id, archive) => {
      ...
      if (!file || !canReadProjectFile(coreRole, {...file,isCurrent:true}) || !await deps.getLead(file.leadId)) return finish("Файл не найден.");
      ...
      if (!visibleFile(file)) return finish("Доступ к архиву закрыт.");
    ...
    try {
      ...
      if (action === "archive") {
        ...
        if (!file || !visibleFile(file)) return finish("Файл не найден.");
      ...
      const canContinueUpload = owned && state?.mode === "batchupload" && (canAddFiles || (canAddPhoto && state.photoOnly));
      if (!canEdit && !(canAddPhoto && action === "photo") && !(canAddFiles && action === "upload") && !canContinueUpload) {
        return finish("Можно просматривать проекты и актуальные файлы. Для фото с объекта используйте кнопку «Добавить фото». Изменения вносит менеджер или дизайнер.");
      }
      ...
      if (state.mode === "batchupload" && action === "savebatch") {
        ...
        for (const file of pending) {
          ...
          const matching = canEdit ? currentFiles.find((item) => item.fileName.toLowerCase() === file.fileName.toLowerCase()) : null;
          const primaryDrawing = currentFiles.find((item) => item.groupKey === "drawings-pdf");
          const groupKey = !canEdit ? `site-file:${randomUUID()}` : matching?.groupKey || (file.fileName.toLowerCase().endsWith(".pdf") && !primaryDrawing ? "drawings-pdf" : `site-file:${randomUUID()}`);
