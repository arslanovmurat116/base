...
export function getMiniAppAuthPlan() {
  return {
    ...
    sessionStrategy: "verified initData mapped to a future Mini App session",
    nextSteps: [
      "Verify initData against TELEGRAM_BOT_TOKEN",
      "Resolve Telegram identity to a BOSE user or client",
      "Issue Mini App session",
      "Bind session to company and unified role model"
    ]
  };
}
...
export async function resolveMiniAppSubject(candidate) {
  ...
  const userResult = await query(
    `
      ...
      where company_id = $1
        and (
          ($2::text is not null and lower(coalesce(telegram_username, '')) = $2)
          or ($3::text is not null and lower(coalesce(full_name, '')) = lower($3))
        )
      limit 1
    `,
    [companyId, normalizedUsername, candidate.displayName || null]
  );
  ...
  const clientResult = await query(
    `
      ...
      where company_id = $1
        and (
          telegram_user_id = $2
          or ($3::text is not null and lower(coalesce(telegram_username, '')) = $3)
        )
      limit 1
    `,
