...
export function verifyTelegramMiniAppInitData(
  ...
  if (!Number.isSafeInteger(authDate) || authDate <= 0 || authDate > Math.floor(Date.now() / 1000) + 30 || ageSeconds > Math.min(3600, Number(maxAgeSeconds) || 3600)) {
    return {
      ok: false,
      verified: false,
      code: "expired",
...
export async function resolveMiniAppSubject(candidate) {
  ...
  const staff = await getAuthorizedStaff(candidate.telegramUserId);
  if (staff) return { subjectType: 'user', subjectId: staff.id, role: staff.role, companyId };
