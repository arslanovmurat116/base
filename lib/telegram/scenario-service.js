...
export async function runBOSEBotScenarioAction({ scenarioId, data = {}, actor = {} }) {
  ...
  switch (scenarioId) {
    case "scenario_discovery": {
      const result = await createScenarioDraftRecord({
        source: "telegram_bot",
        telegramUserId: normalizedActor.telegramUserId,
        chatId: normalizedActor.chatId,
      ...
      if (!result?.ok || !result?.draft) {
        await trackScenarioAnalytics(normalizedActor, "scenario_draft_failed", {
          scenarioId,
      ...
      await trackScenarioAnalytics(normalizedActor, "scenario_draft_created", {
        scenarioId,
